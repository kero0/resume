...
- Primary researcher into neuromorphic computing, using Intel's Loihi platform. Designed and wrote tests and gathered data to compare accuracy, efficiency, and speed against traditional computing and quantum computing for our company's use cases
...
- Wrote API microservice to serve as PoC for Ford's potential IoT platform
...
- Wrote PoC library for securely allowing upfitter programs to communicate both with the vehicle and add in modules
...
#+begin_multicols
- Programming Languages: Assembly (ARM), Bash, C, C++, JavaScript, MATLAB, Python, Rust, SystemVerilog, ...
